<template>
  ...
  <div
    ...
    :key="surah.number"
  >
    <router-link :to="`surat/${surah.number}`">
      <div class="flex">
        <svg
        ...
        <div class="ml-3">
          <h3 class="text-teal-500">{{ surah.englishName }}</h3>
          <p class="text-[12px] text-slate-400">
            {{ convertToTypeSurah(surah.revelationType) }},
            {{ surah.numberOfAyahs }} Ayat
          </p>
        </div>
        <div class="ml-auto my-auto">
          <p class="text-2xl text-teal-500">{{ surah.name }}</p>
        </div>
      </div>
    </router-link>
  </div>
...
export default {
  setup() {
    ...
    const convertToTypeSurah = (place) => {
      return place == "Mecca" ? "Makkiyah" : "Madaniyyah";
    };

    // untuk filter surah
    const searchSurah = computed(() => {
      return surahs.value.filter((surah) =>
        surah.englishName.toLowerCase().includes(search.value.toLowerCase())
      );
    });
